...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Examples:"
            echo "  ./test.sh                            # Run all tests (uses existing local.settings.json)"
            echo "  ./test.sh --inprocess-only           # Run only in-process tests"
            echo "  ./test.sh --queue-url https://sqs.us-east-1.amazonaws.com/123/my-queue \\"
